...
async function escapeERC20(): Promise<void> {
    ...
    let messagePasserStorageSlot = (await l2Provider.send('eth_getProof', [
      config.l2ToL1MessagePasserAddress,
      ["0xf"],
      stripHexLeadingZeros(l2OutputBlockNumber.toHexString())
    ])).storageProof[0].value

    let outputRoot = ethers.utils.keccak256(ethers.utils.defaultAbiCoder.encode([ "uint", "uint","uint","uint"],["0x0",l2Block.stateRoot,messagePasserStorageSlot,l2Block.hash]));
    ...
    let escapeTx = await L1StandardBridgeContract.populateTransaction.escapeERC20(
        ...
        config.l2ERC20ForEscape!,
        config.erc20Upgradable! === "true",
        {
            version: ethers.constants.HashZero,
            stateRoot: l2Block.stateRoot,
            messagePasserStorageRoot: messagePasserStorageSlot,
